...
<Button href="https://chromewebstore.google.com/detail/replay-recorder/lkbmpddckbjbfaekcjacjgpehgaaijhh?authuser=2&hl=en" variant="primary">Add Replay to Chrome</Button>
...
Recording adds some overhead, but in most cases it's not noticeable during normal app usage. If you're testing performance-sensitive flows, use the Replay browser via the CLI for lower overhead.
</AccordionItem>

<AccordionItem title="Who can see my recordings?">
Recordings are private to your account by default. You can share a replay by sending the link, or manage access in your team settings.
</AccordionItem>

<AccordionItem title="Can I record localhost?">
Yes. The extension works on any page Chrome can open, including `localhost` and local network addresses, but it is better at debugging dev preview urls because they include more information for Replay to analyze, for example source maps. 
</AccordionItem>
