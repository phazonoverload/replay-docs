...
<Callout type="note">
  Other available skills include `replay-cli` (record via CLI),
  `replay-playwright` (record Playwright tests), and `replay-cypress` (record
  Cypress tests). These let agents create recordings on your behalf.
</Callout>
...
<Callout type="note">
  **React 18.3 users:** React analysis tools (render tracking, component
  inspection, performance profiling) require production sourcemaps for React
  itself. You'll need to add the `@acemarke/react-prod-sourcemaps` build plugin.
  See [React Version
