...
Add the Replay plugin to your Playwright configuration. See the detailed setup guide:

- [Record Playwright tests](/basics/getting-started/record-your-playwright-tests)

## View recordings in CI

Once configured, every test run produces recordings that are uploaded automatically. Failed tests show up in the [Test Suite Dashboard](/basics/test-suites/overview) with links to inspect each recording.

</Steps>

<Callout type="note">

**Agent-driven setup:** AI coding agents can help configure test recording using the `replay-playwright` skill. Install skills with `npx @replayio/replay skills add`.
...
<QuickLink
  title="Automate with the Protocol API"
  icon="jumptocode"
  href="/reference/integrations/replay-apis/replay-protocol"
  description="Use the Replay Protocol directly for custom integrations, scripts, and automated analysis."
/>
...
- **Debugging intermittent test failures?** Set up **Playwright/E2E recording** in CI so you automatically have recordings when tests fail.
